...
namespace INIFileMan
{
    public class INIFile : IINIFile
    {
        ...
        private readonly IniParser.IniDataParser _INIParser;
        /// <summary>
        ...
        public INIFile(string iniPath, bool forceCreate = false)
        {
            ...
            if (File.Exists(iniPath) || forceCreate)
            {
                ...
                _INIParser = new IniParser.IniDataParser();
                //Configuration = new IniParser.Configuration.IniParserConfiguration();
                //Configuration.sp = ""; // no spaces before and after keyvalue splitter char

                _INIData = _INIParser.Parse(File.ReadAllText(_iniPath.FullName));
            }
            //else
            //{
        ...
        public void SetKey(string section, string key, string value, bool doWriteFile = true)
        {
            ...
            if (string.IsNullOrEmpty(section))
            {
                if (!_INIData.Global.Contains(key) || _INIData.Global[key] != value)
                {
            ...
            else
            {
                if (!SectionExists(section))
                {
                    _INIData.Sections.Add(section);
                }

                if (!_INIData[section].Contains(key) || _INIData[section][key] != value)
                {
        ...
        public bool KeyExists(string key, string section = null)
        {
            ...
            if (string.IsNullOrEmpty(section))
            {
                return _INIData.Global.Contains(key);
            }
            else
            {
                if (!SectionExists(section))
                ...
                return _INIData[section].Contains(key);
            }
        ...
        public void DeleteSection(string section/* = null*/, bool doWriteFile = true)
        {
            ...
            if (SectionExists(section))
            {
                _INIData.Sections.Remove(section);
        ...
        public IEnumerable<KeyValuePair<string, string>> GetSectionKeyValuePairs(string section)
        {
            ...
            else
            {
                ...
                {
                    if (!string.IsNullOrEmpty(keyData.Key))
                    {
        ...
        {
            return !string.IsNullOrEmpty(section) && _INIData.Sections.Contains(section);
        }
        ...
        public void WriteFile(bool doWriteFile = true)
        {
            if (doWriteFile)
            {
                ...
                _INIData.
                _INIParser.(_iniPath, _INIData, new UTF8Encoding(false));
            }
        }
        ...
        {
            var keyData = new KeyValuePair(key, section);
